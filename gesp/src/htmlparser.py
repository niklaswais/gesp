...
class DecisionHTMLParser(HTMLParser):
    ...
    ecli_codes = {}
    gerichts_code_land = {}
    _ecli_csv_text = files("gesp").joinpath("data/ECLI-Gerichtscodes.csv").read_text(encoding="utf-8")
    csvfile = csv.reader(_ecli_csv_text.splitlines())
    for land, gerichtsname, code in csvfile:
        if gerichtsname in ecli_codes:
            continue  ### LG FFM has two abbreviations ...
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "div" and ("id", "bb-footer-bar") in attrs:
            self.bbfooter = True

        if tag == "rechtsmittelinstanz":
            self.inRechtsmittelInstanz = True
        if tag == "vorinstanz":
            self.vorInstanz = True
        if tag == "hr":
            self.inBody = False
        if (tag == "table") and (
            (("class", "TableRahmenkpl") in attrs)
            or (("class", "kopfTabelle") in attrs)
            or (("style", "width:100%;border-spacing:5px;") in attrs)
        ):
            self.inTabelle = True
        if self.printTagData:
            print(f"\tTag {tag} attrs {attrs} in Tabelle {self.inTabelle}")

        if self.inTabelle and tag == "table":
            self.tabellentiefe += 1
        if tag == "a" and ("name", "DocInhaltEnde") in attrs:
            self.bodyEnde = True

        if self.printTagData:
            print(f"\tTabelle {self.inTabelle}")
    ...
    def trenne_gericht_und_kammer(self, bezeichnung):
        ...
        for block in bezeichnung.split(" "):
            ...
            if block[0].isnumeric():
                nochGerichtsname = False
            if (
                self.gericht != ""  ## Es gibt ein Gericht das "Vergabekammer" heißt
                and (
                    "senat" in block.lower()
                    or "kammer" in block.lower()
                    or "kleine" in block.lower()
                    or "große" in block.lower()
                    or "schwurgericht" in block.lower()
                    or "ausschuss" == block.lower()
                    or "präsident" == block.lower()
                    or "generalstaatsanwalt" == block.lower()
            ...
            if nochGerichtsname:
                if len(self.gericht) > 0:
                    self.gericht += " "
                self.gericht += block
            else:
                if len(self.spruchkoerper) > 0:
                    self.spruchkoerper += " "
    ...
    def handle_data(self, data):
        tag = self.start_tag
        if self.printTagData:
            print("Some data [%s] : %s" % (tag, data))

        if self.mode in ("bund", "by"):
            # print(tag)
            # print(data)
            # print("====================================================================")
            if self.inRechtsmittelInstanz or self.vorInstanz:
        ...
        ##
            ...
            if tag == "div":
                if self.feldbezeichnungsModus:
                    ...
                    self.feldname = ""
            elif tag in ("p", "b", "strong", "span", "em", "sub", "sup", "ul", "del", "ins") or (
                (self.NRWneu == True) and (tag in ("u", "li", "i"))
            ):  ## links? aka a href?
                if data.isspace():
                    return
                if data == "(Hier Freitext: Tatbestand, Gründe etc.)" and self.NRWneu == True:
                    return
            ...
            if tag == "strong":
                ...
                if (
                    self.randnummerLink == False
                    and self.inTabelle == False
                    and self.gericht != ""
                    and not self.bodyEnde
                ):
                    self.text += " " + data.strip()
            ...
            # print(self.gericht)
                ...
                if len(self.aktenzeichen) > 0 and not self.endeDerEntscheidung:
                    if "Diese Entscheidung enthält keinen zur Veröffentlichung bestimmten Leitsatz." in data:
                        return
                    if len(self.text) > 0:
                        if tag == "a":
                            self.text += " "
                ...
                if "Ende der Entscheidung" in data:
                    self.endeDerEntscheidung = True
        elif self.mode in ("bb"):
            if len(data) == 0 or data.isspace():
                return
            if tag == "h6" and data == "Service":
                self.bbfooter = True
            if self.bbfooter == True:
                return
            if tag == "th":
    ...
    def complete_attributes(self):
        ...
        if self.dokumenttyp == "Bes":
            self.dokumenttyp = "Beschluss"
        if self.dokumenttyp == "Urt":
            self.dokumenttyp = "Urteil"
        ...
        if self.gerichtstyp == "VerfGH":
            self.gerichtstyp = "Verfassungsgerichtshof"
            gerichtstypgeändert = True
        if self.gerichtstyp == "BayObLG":
            self.gerichtstyp = "Bayerisches Oberstes Landesgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "OLG":
            self.gerichtstyp = "Oberlandesgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "OVG":
            self.gerichtstyp = "Oberverwaltungsgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "LSG":
            self.gerichtstyp = "Landessozialgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "VGH":
            self.gerichtstyp = "Verwaltungsgerichtshof"
            gerichtstypgeändert = True
        if self.gerichtstyp == "LArbG":
            self.gerichtstyp = "Landesarbeitsgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "VG":
            self.gerichtstyp = "Verwaltungsgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "SG":
            self.gerichtstyp = "Sozialgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "AG":
            self.gerichtstyp = "Amtsgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "LG":
            self.gerichtstyp = "Landgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "FG":
            self.gerichtstyp = "Finanzgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "KG":
            self.gerichtstyp = "Kammergericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "ArbG":
            self.gerichtstyp = "Arbeitsgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "BFH":
            self.gerichtstyp = "Bundesfinanzhof"
            gerichtstypgeändert = True
        if self.gerichtstyp == "VerfG":
            self.gerichtstyp = "Verfassungsgericht"
            gerichtstypgeändert = True
        if self.gerichtstyp == "DGH":
            self.gerichtstyp = "Dienstgerichtshof"
            gerichtstypgeändert = True
        if self.gerichtstyp == "LGHE":
            self.gerichtstyp = "Landgericht Frankfurt"
            gerichtstypgeändert = True
        ...
        if self.ecli == "":
            ...
            if self.ecli_codes[self.gericht] in ("BGH", "BVerwG", "BSG", "BAG", "BFH", "BPatG"):
                typcode = ""
                if self.dokumenttyp == "Urteil":
                    typcode = "U"
                if self.dokumenttyp == "Versäumnisurteil":
                    typcode = "U"
                if self.dokumenttyp == "Anerkenntnisurteil":
                    typcode = "U"
                if self.dokumenttyp == "Teilurteil":
                    typcode = "U"
                if self.dokumenttyp == "Beschluss":
                    typcode = "B"
                if self.dokumenttyp == "Vorlagebeschluss":
                    typcode = "B"
                if self.dokumenttyp == "Gerichtsbescheid":
                    typcode = "G"
                if self.dokumenttyp == "EuGH-Vorlage":
                    if self.ecli_codes[self.gericht] == "BFH":
                        ...
                        typcode = "B"
                if self.dokumenttyp == "Verfügung":
                    typcode = "V"
                if self.dokumenttyp == "Sonstige":
                    typcode = "S"

                if typcode == "B" and self.ecli_codes[self.gericht] == "BFH":
                    if "Aussetzung der Vollziehung" in self.text:
                        typcode += "A"
                ...
                if self.ecli_codes[self.gericht] in ("BAG"):
                    self.ecli += "."
                ...
                if self.ecli_codes[self.gericht] != "BPatG":
                    format_aktenzeichen = format_aktenzeichen.upper()
                format_aktenzeichen = (
                    format_aktenzeichen.replace("Ä", "AE").replace("Ö", "OE").replace("Ü", "UE").split(",", 1)[0]
                )
                ...
                if self.ecli_codes[self.gericht] in ("BSG"):
                    ...
                    format_aktenzeichen = (
                        format_aktenzeichen.replace("/", ".").replace("(", ".").replace(")", ".").replace("-", "")
                    )
                ...
                if self.ecli_codes[self.gericht] in ("BGH", "BVerwG", "BFH", "BAG", "BPatG"):
                    self.ecli += "."
                ...
                if "PBv" in self.aktenzeichen:
                    ...
                    if "Kammer" in self.spruchkoerper:
                        self.ecli += "k"
                    else:
                        self.ecli += "s"
                ...
                for _ in range(4 - len(res.group(4))):
                    format_aktenzeichen += "0"
                ...
                az_splitted = (
                    self.aktenzeichen.upper().replace("Ä", "AE").replace("Ö", "OE").replace("Ü", "UE").split(",", 1)
                )
                ...
                prev = " "
                azblock = ""
                if self.ecli_codes[self.gericht] == "VERFGHT":
                    azblock = "VERFGH"  # Verfassungsbericht Thürigen fügt Gerichtsnamen ins AZ intu anders ...
                if self.ecli_codes[self.gericht] == "STGHNI":
                    azblock = "STGH"  # Verfassungsbericht Thürigen fügt Gerichtsnamen ins AZ intu anders ...
                ...
                if self.ecli_codes[self.gericht] in [
                    "OVGNRW",
                    "VGK",
                    "VGD",
                    "VFGHNRW",
                ...
                for block in re.split(
                    splitstring, format_aktenzeichen
                ):  ## if the AZ contains a dot, this seems to be kept usually
                    if len(block) == 0:
                        continue
                    if block[0].isalpha() and prev[-1].isalpha():
                        azblock += "."
                    if block[0].isnumeric() and prev[-1].isnumeric():
                        azblock += "."
                    azblock += block
                    prev = block
