...
class SpdrHE(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "fg" in self.courts:
            self.filter.append("hessisches finanzgericht")
        if "lag" in self.courts:
            self.filter.append("hessisches landesarbeitsgericht")
        if "lg" in self.courts:
            self.filter.append("lg")
        if "lsg" in self.courts:
            self.filter.append("hessisches landessozialgericht")
        if "olg" in self.courts:
            self.filter.append("olg")
        if "ovg" in self.courts:
            self.filter.append("hessischer verwaltungsgerichtshof")
        if "sg" in self.courts:
            self.filter.append("sg")
        if "vg" in self.courts:
            self.filter.append("vg")
    ...
    def parse(self, response):
        ...
        time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
        body = (
            '{"searchTasks":{"RESULT_LIST":{"start":1,"size":25,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":25,"size":27},"FAST_ACCESS":{},"SEARCH_WORD_HITS":{}},"filters":{"CATEGORY":["Rechtsprechung"]},"searches":[],"clientID":"bshe","clientVersion":"bshe - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}'
            % (date, time)
        )
        yield scrapy.Request(
            url=url,
            ...
            meta={"batch": 26},
            dont_filter=True,
            callback=self.parse_scrolldown,
        )
    ...
    def parse_scrolldown(self, response):
        ...
        if "resultList" in results:
            ...
            time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
            body = (
                '{"searchTasks":{"RESULT_LIST":{"start":%s,"size":25,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":%s,"size":27},"FAST_ACCESS":{}},"filters":{"CATEGORY":["Rechtsprechung"]},"searches":[],"clientID":"bshe","clientVersion":"bshe - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}'
