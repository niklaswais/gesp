...
class SpdrBund(scrapy.Spider):
    ...
    def __init__(
        ...
        if "straf" in domains and "bgh" not in courts:
            courts.append("bgh")
    ...
    def parse(self, response):
        if not self.courts or "bverfg" in self.courts:
            ...
            yield scrapy.Request(
                ...
                headers=config.HEADERS | {"Referer": "https://www.bundesverfassungsgericht.de/"},
            )
        for item in response.xpath("//item"):
            link = item.xpath("link/text()").get()
            y = {
    ...
    def parse_bverfg_collection(self, response):
        for link in response.xpath("//a/@href"):
            if "/Entscheidungen/Liste/" in link.get():
                yield scrapy.Request(
                    url=response.urljoin(link.get()), callback=self.parse_bverfg_list, headers=config.HEADERS
                )
    ...
    def parse_bverfg_list(self, response):
        for row in response.xpath("//tr"):
            ...
            if az_column:
                ...
                for az in azs.split(", "):
                    az = re.sub(" +$", "", az)
                    if az not in self.bverfg_az_bmj:
                        needed = az
                        break
            if not needed:
                continue
            for link in row.xpath(".//td[1]/a/@href"):
                if "/Entscheidungen/" in link.get():
                    ...
                    if not re.match(pat, date_raw):
                        continue
                    date_raw = re.sub("^(Beschluss|Urteil) vom (" + pat_d + ")$", "\\2", date_raw)
                    date_ymd = (
                        re.sub(pat_d, "\\3", date_raw)
                        + monate[re.sub(pat_d, "\\2", date_raw)]
                        + re.sub(pat_d, "\\1", date_raw).zfill(2)
                    )
                    yield {
                        "wait": self.wait,
                        "date": date_ymd,
