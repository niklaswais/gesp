    ...
    async def start(self):
        ...
        if self.courts:
            if "ag" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/Amtsgerichte/Ordentliche%20Gerichtsbarkeit")
            if "arbg" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/Arbeitsgerichte/Arbeitsgerichtsbarkeit")
            ...
            if "lg" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/Landgerichte/Ordentliche%20Gerichtsbarkeit")
            if "lsg" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/LSG%20M%C3%BCnchen/Sozialgerichtsbarkeit")
            if "olg" in self.courts:
                ...
                start_urls.append(
                    "LEVEL2RSPRTREENODE/BayObLG%20M%C3%BCnchen/Ordentliche%20Gerichtsbarkeit"
                )  # Sonderfall Bayern (BayObLG)...
            if "ovg" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/VGH%20M%C3%BCnchen/Verwaltungsgerichtsbarkeit")
            if "sg" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/Sozialgerichte/Sozialgerichtsbarkeit")
            if "vg" in self.courts:
                start_urls.append("LEVEL2RSPRTREENODE/Verwaltungsgerichte/Verwaltungsgerichtsbarkeit")
            if "verfgh" in self.courts:
                start_urls.append("LEVEL1RSPRTREENODE/Verfassungsgerichtsbarkeit")
        else:
    ...
    def parse(self, response):
        if not response.xpath("//div[@id='hinweis']"):  # Bei Hinweis Seite ohne Suchergebnisse
            for item in response.xpath("//li[@class='hitlistItem']"):
                ...
                if "straf" in self.domains and "zivil" not in self.domains:
                    output("filter (-s by -d straf) not yet implemented", "warn")
                    # Ausbauen ....
                elif "zivil" in self.domains and "straf" not in self.domains:
        ...
        if response.xpath("//a[text()='→']"):
            yield response.follow(
                response.xpath("//a[text()='→']/@href").get(),
                dont_filter=True,
