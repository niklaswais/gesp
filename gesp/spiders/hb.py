...
class SpdrHB(scrapy.Spider):
    ...
    async def start(self):
        ...
        if not self.courts or "lag" in self.courts:
            url = (
                "https://www.landesarbeitsgericht.bremen.de/entscheidungen/entscheidungsuebersicht-11508?max=100&skip=0"
            )
    ...
    def parse(self, response):
        for td in response.xpath("//tr/td[@class='dotright'][1]"):
            if "(pdf" not in td.xpath(".//following-sibling::td/a[1]/text()").get():
                continue
