...
class SpdrHH(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "olg" in self.courts:
            self.filter.append("hanseatisches oberlandesgericht")
        if "ovg" in self.courts:
            self.filter.append("hamburgisches oberverwaltungsgericht")
    ...
    def parse(self, response):
        ...
        time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
        body = (
            '{"searchTasks":{"RESULT_LIST":{"start":1,"size":25,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":25,"size":27},"FAST_ACCESS":{},"SEARCH_WORD_HITS":{}},"filters":{"CATEGORY":["Rechtsprechung"]},"searches":[],"clientID":"bsha","clientVersion":"bsha - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}'
            % (date, time)
        )
        yield scrapy.Request(
            url=url,
    ...
    def parse_nextpage(self, response):
        ...
        if "resultList" in results:
            ...
            body = (
                '{"searchTasks":{"RESULT_LIST":{"start":%s,"size":25,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":%s,"size":27},"FAST_ACCESS":{}},"filters":{"CATEGORY":["Rechtsprechung"]},"searches":[],"clientID":"bsha","clientVersion":"bsha - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}'
