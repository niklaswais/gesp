...
class SpdrBE(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "fg" in self.courts:
            self.filter.append("finanzgericht")
        if "lag" in self.courts:
            self.filter.append("larbg")
        if "lg" in self.courts:
            self.filter.append("lg")
        if "lsg" in self.courts:
            self.filter.append("landessozialgericht")
        if "olg" in self.courts:
            self.filter.append("kg")
    ...
    def parse(self, response):
        ...
        time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
        body = (
            '{"searchTasks":{"CATEGORY_HITS":{},"RESULT_LIST":{"start":1,"size":51,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":52,"size":50},"SEARCH_WORD_HITS":{}},"filters":{"CATEGORY":["Rechtsprechung"]},"searches":[],"clientID":"bsbe","clientVersion":"bsbe - V06_07_00 - 23.06.2022 11:20","r3ID":"s%sT%sZ"}'
            % (date, time)
        )
        yield scrapy.Request(
            url=url,
    ...
    def parse_scrolldown(self, response):
        ...
        if "resultList" in results:
            ...
            time = str(datetime.datetime.now(datetime.timezone.utc).time())[0:-3]
            body = (
                '{"searchTasks":{"RESULT_LIST":{"start":%s,"size":50,"sort":"date","addToHistory":true,"addCategory":true},"RESULT_LIST_CACHE":{"start":%s,"size":50},"FAST_ACCESS":{}},"filters":{"CATEGORY":["Rechtsprechung"]},"searches":[],"clientID":"bsbe","clientVersion":"bsbe - V06_07_00 - 23.06.2022 11:20","r3ID":"%sT%sZ"}'
                % (batch, batch + 50, date, time)
            )
            yield scrapy.Request(
                url=url,
                method="POST",
